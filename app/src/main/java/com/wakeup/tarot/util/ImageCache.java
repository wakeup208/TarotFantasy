...
//						}
	...
	/**
	 * Clear the disk cache associated with this ImageCache object.
	 */
				...
				try {
					...
					if (BuildConfig.DEBUG) {
						Log.d(TAG, "Disk cache cleared: "
								+ mCacheParams.diskCacheDir.getPath());
					}
				} catch (IOException e) {
	 ...
	 */
	public static void deleteDiskCacheDir() {
		final String cacheDir = Environment.MEDIA_MOUNTED.equals(Environment
				.getExternalStorageState()) || !isExternalStorageRemovable() ? getExternalCacheDir(
				CONTEXT).getPath()
				: CONTEXT.getCacheDir().getPath();
